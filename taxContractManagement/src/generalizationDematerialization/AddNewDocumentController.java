...
import static dematerialization.AddDocumentController.path;
 ...
 * @author Deeplight
 */
public class AddNewDocumentController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        addFieldBtn.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> {
            String fldName = fieldName.getText();
            String dataSet = "Data";
            if (fldName.length() == 0) {

                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setHeaderText(null);
                alert.setContentText("Veuillez vous entrez le nom de document ");
                alert.showAndWait();

            } else {
                try {
                    query = "INSERT INTO `fieldsname`(`document_name`, `field_name`) VALUES (?,?)";
                    connection = DbConnect.getConnect();
                    preparedStatement = connection.prepareStatement(query);
                    preparedStatement.setString(1, documentName.getText());
                    preparedStatement.setString(2, fieldName.getText());
                    preparedStatement.execute();

                    File file1 = new File("C:\\Users\\Deeplight\\OneDrive\\Documents\\NetBeansProjects\\taxContractManagement-main\\taxContractManagement\\src\\Dataset\\" + documentName.getText() + "\\" + fieldName.getText());

                    if (!file1.exists()) {
                        if (file1.mkdir()) {

                            CreateDataset.addtoCsv((int) imageView.getFitWidth(), (int) zone.getWidth(), "C:\\Users\\Deeplight\\OneDrive\\Documents\\NetBeansProjects\\taxContractManagement-main\\taxContractManagement\\src\\Dataset\\" + documentName.getText() + "\\" + fieldName.getText() + "\\Width.csv");
                            CreateDataset.addtoCsv((int) imageView.getFitWidth(), (int) zone.getX(), "C:\\Users\\Deeplight\\OneDrive\\Documents\\NetBeansProjects\\taxContractManagement-main\\taxContractManagement\\src\\Dataset\\" + documentName.getText() + "\\" + fieldName.getText() + "\\Xoffset.csv");
                            CreateDataset.addtoCsv((int) imageView.getFitWidth(), (int) zone.getHeight(), "C:\\Users\\Deeplight\\OneDrive\\Documents\\NetBeansProjects\\taxContractManagement-main\\taxContractManagement\\src\\Dataset\\" + documentName.getText() + "\\" + fieldName.getText() + "\\Height.csv");
                            CreateDataset.addtoCsv((int) imageView.getFitWidth(), (int) zone.getY(), "C:\\Users\\Deeplight\\OneDrive\\Documents\\NetBeansProjects\\taxContractManagement-main\\taxContractManagement\\src\\Dataset\\" + documentName.getText() + "\\" + fieldName.getText() + "\\Yoffset.csv");
        ...
        addDocBtn.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> {
            ...
            if (docName.length() == 0) {
                ...
                try {
                    query = "INSERT INTO `documentname`(`name`) VALUES (?)";
                    connection = DbConnect.getConnect();
                    preparedStatement = connection.prepareStatement(query);
                    preparedStatement.setString(1, documentName.getText());
                    preparedStatement.execute();
                    ...
                    File file2 = new File("C:\\Users\\Deeplight\\OneDrive\\Documents\\NetBeansProjects\\taxContractManagement-main\\taxContractManagement\\src\\Dataset\\" + documentName.getText());
                    if (!file2.exists()) {
                        if (file2.mkdir()) {
